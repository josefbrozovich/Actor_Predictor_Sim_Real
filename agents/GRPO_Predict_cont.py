...
class GRPO_Predict_cont:
    ...
    def update(self, rewards, terminates):
        ...
        for _ in range(self.num_updates):
            mu, sigma, = self.model.forward(trajectory)
            dist = torch.distributions.Normal(mu, sigma)
            new_log_probs = dist.log_prob(action_taken).sum(dim=-1)
            ...
            per_token_loss = -(torch.min(surr1, surr2))-self.alpha*entropy#-self.beta*kl
